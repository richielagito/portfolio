...
const projects = [
  {
    ...
    tagline:
      "A modern, interactive landing page for Kopi Kenangan, showcasing their brand and products through engaging storytelling.",
  },
  {
...
const Chapter = ({ project, index, scrollYProgress }) => {
  ...
  const opacity = useTransform(
    scrollYProgress,
    [start, snapIn, holdStart, holdEnd, snapOut, end],
    [0, 0.6, 1, 1, 0.6, 0],
  );
  ...
  const taglineOpacity = useTransform(
    scrollYProgress,
    [tagStart, tagVisible, holdEnd, snapOut, end],
    [0, 1, 1, 0.5, 0],
  );
  const taglineY = useTransform(
    scrollYProgress,
    [tagStart, tagVisible, holdEnd, end],
    [20, 0, 0, -20],
  );
    ...
    >
      <motion.div className="flex flex-col md:flex-row md:items-center md:gap-12 lg:gap-20 items-center justify-center w-full max-w-6xl">
        ...
        <div className="flex flex-col items-center md:items-start md:w-[40%] shrink-0">
          <motion.span
            className="text-xs md:text-sm font-medium tracking-[0.25em] uppercase text-neutral-500 mb-4"
            style={{ y: textY }}
          >
            {project.category}
          </motion.span>
          ...
          <motion.h2
            className="text-5xl md:text-6xl lg:text-7xl font-display font-semibold tracking-tighter text-foreground mb-6 md:mb-8 text-center md:text-left leading-[1]"
            style={{ y: textY }}
          >
            {project.title}
          </motion.h2>
          ...
          <motion.p
            className="text-base md:text-lg text-neutral-400 font-light text-center md:text-left max-w-md"
        ...
        <motion.div
          className="relative w-[85vw] md:w-[55%] max-w-2xl aspect-video rounded-xl overflow-hidden scrollytelling-glow mt-8 md:mt-0"
          style={{
            scale: imgScale,
            opacity: imgOpacity,
...
const Scrollytelling = () => {
  ...
  // Lock viewport height once on mount — immune to Safari toolbar changes
  const [stableVh, setStableVh] = useState(null);
  ...
  // Use stable pixel height: 400 * initialViewportHeight
  const sectionHeight = stableVh ? stableVh * TOTAL_HEIGHT_VH / 100 : `${TOTAL_HEIGHT_VH}vh`;
  const stickyHeight = stableVh ? `${stableVh}px` : '100dvh';

  return (
    <section
      ref={containerRef}
      className="relative"
      style={{ height: typeof sectionHeight === 'number' ? `${sectionHeight}px` : sectionHeight }}
    >
      <motion.div
        className="sticky top-0 w-full overflow-hidden"
        style={{ opacity: sectionOpacity, height: stickyHeight }}
      >
        ...
        {projects.map((project, index) => (
          <Chapter
            ...
            project={project}
            index={index}
            scrollYProgress={animatedProgress}
          />
        ))}
...
const IntroOverlay = ({ scrollYProgress }) => {
  ...
  return (
    <motion.div
      className="absolute inset-0 flex flex-col items-center justify-center px-6 md:px-16 pointer-events-none"
      style={{ opacity, y, scale, filter: blurFilter }}
    >
      <span className="text-xs md:text-sm font-medium tracking-[0.25em] uppercase text-neutral-500 mb-4">
        What I Can Do
      </span>
...
const ProgressDots = ({ scrollYProgress }) => {
  ...
  const dotsOpacity = useTransform(
    scrollYProgress,
    [firstStart - 0.03, firstStart + 0.03, lastEnd - 0.03, lastEnd],
    [0, 1, 1, 0],
  );
  ...
  return (
    <motion.div
      className="absolute right-8 top-1/2 -translate-y-1/2 hidden md:flex flex-col items-center gap-4 z-20"
      style={{ opacity: dotsOpacity }}
    >
      <div className="relative w-px h-20 bg-neutral-800 rounded-full overflow-hidden">
        <motion.div
          className="absolute top-0 left-0 w-full bg-gradient-to-b from-white to-neutral-400 rounded-full"
...
const DotIndicator = ({ scrollYProgress, chapterMid, chapterSize }) => {
  const dotOpacity = useTransform(
    scrollYProgress,
    [
      chapterMid - chapterSize * 0.35,
      chapterMid - chapterSize * 0.15,
      chapterMid + chapterSize * 0.15,
      chapterMid + chapterSize * 0.35,
    ],
    [0.3, 1, 1, 0.3],
