  ...
  return (
    <>
      ...
      >
        ...
        <button
          onClick={toggleMenu}
          className="group flex flex-col gap-1.5 w-8 items-end cursor-pointer mix-blend-difference z-50 focus:outline-none"
          aria-label="Toggle Menu"
        >
          <span
            className={`block h-0.5 bg-white transition-all duration-300 ${isMenuOpen ? "w-8 rotate-45 translate-y-2" : "w-8"}`}
          ></span>
          <span
            className={`block h-0.5 bg-white transition-all duration-300 ${isMenuOpen ? "opacity-0" : "w-6 group-hover:w-8"}`}
          ></span>
          <span
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div
            ...
            animate="open"
            exit="closed"
            className="fixed inset-0 bg-neutral-950 z-40 flex flex-col justify-center items-center"
          >
            <div className="container mx-auto px-6 h-full flex flex-col md:flex-row">
              ...
              <div className="hidden md:flex flex-col justify-end pb-20 w-1/3">
                <h3 className="text-neutral-500 uppercase text-sm tracking-widest mb-6">
                  Contact
                </h3>
                <a
                  href="mailto:richielagito1@gmail.com"
                  className="text-xl text-neutral-300 hover:text-white transition-colors mb-2"
                >
                  ...
                  {navLinks.map((link, index) => (
                    ...
                    >
                      <Link
                        href={link.href}
                        onClick={toggleMenu}
                        className="block text-5xl md:text-8xl font-black text-neutral-400 hover:text-white transition-colors tracking-tighter group relative"
                      >
                        {link.title}
                      </Link>
                    </motion.div>
